...
# Your name: Hailey Hartz
# Your student id: 21282080
# Your email: hhartz@umich.edu
...
def make_players_table(data, cur, conn):
    ...
    for player in players:
        ...
        cur.execute('SELECT id FROM Positions WHERE position = ?', (position_str,))
        position_id = int(cur.fetchone()[0])
        cur.execute('INSERT OR IGNORE INTO Players (id,name,position_id,birthyear,nationality) VALUES (?,?,?,?,?)', (player_id, name, position_id, birthyear, nationality))
        conn.commit()
...
def birthyear_nationality_search(age, country, cur, conn):
    player_list = []
    year_cutoff = 2023 - age
    cur.execute('SELECT name, nationality, birthyear FROM Players WHERE nationality = ? AND birthyear < ?', (country, year_cutoff))
    for row in cur:
...
class TestAllMethods(unittest.TestCase):
    ...
    def test_make_winners_table(self):
        ...
        self.assertEqual(winners_list[-1][1], 'Leicester City FC')
